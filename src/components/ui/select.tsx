...
>(({ className, children, ...props }, ref) => (
  <SelectPrimitive.Trigger
    ref={ref}
    className={cn(
      "[&>span]:line-clamp-1 bg-transparent border border-input cursor-pointer data-[placeholder]:text-muted-foreground disabled:cursor-not-allowed disabled:opacity-50 flex focus:outline-none focus:ring-1 focus:ring-ring h-9 items-center justify-between px-3 py-2 ring-offset-background rounded-md shadow-sm text-sm w-full whitespace-nowrap",
      className,
    )}
    {...props}
  >
...
>(({ className, children, ...props }, ref) => (
  <SelectPrimitive.Item
    ref={ref}
    className={cn(
      "cursor-default data-[disabled]:opacity-50 data-[disabled]:pointer-events-none flex focus:bg-accent focus:text-accent-foreground items-center outline-none pl-2 pr-8 py-1.5 relative rounded-sm select-none text-sm w-full",
      className,
    )}
    {...props}
  >
    <span className="absolute flex h-3.5 items-center justify-center right-2 w-3.5">
