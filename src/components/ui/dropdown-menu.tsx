...
>(({ className, inset, children, ...props }, ref) => (
  <DropdownMenuPrimitive.SubTrigger
    ref={ref}
    className={cn(
      "[&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg]:size-4 cursor-default data-[state=open]:bg-accent flex focus:bg-accent gap-2 items-center outline-none px-2 py-1.5 rounded-sm select-none text-sm",
      inset && "pl-8",
      className,
    )}
    {...props}
...
>(({ className, sideOffset = 4, ...props }, ref) => (
  <DropdownMenuPrimitive.Portal>
    <DropdownMenuPrimitive.Content
      ...
      className={cn(
        "bg-popover border max-h-[var(--radix-dropdown-menu-content-available-height)] min-w-[8rem] overflow-x-hidden overflow-y-auto p-1 rounded-md shadow-md text-popover-foreground z-50",
...
>(({ className, inset, ...props }, ref) => (
  <DropdownMenuPrimitive.Item
    ref={ref}
    className={cn(
      "[&>svg]:shrink-0 [&>svg]:size-4 cursor-default data-[disabled]:opacity-50 data-[disabled]:pointer-events-none flex focus:bg-accent focus:text-accent-foreground gap-2 items-center outline-none px-2 py-1.5 relative rounded-sm select-none text-sm transition-colors",
      inset && "pl-8",
      className,
    )}
    {...props}
...
>(({ className, ...props }, ref) => (
  <DropdownMenuPrimitive.Separator
    ref={ref}
    className={cn("-mx-1 bg-muted h-px my-1", className)}
    {...props}
  />
));
...
const DropdownMenuShortcut = ({ className, ...props }: React.HTMLAttributes<HTMLSpanElement>) => {
  return (
    <span className={cn("ml-auto opacity-60 text-xs tracking-widest", className)} {...props} />
  );
};
